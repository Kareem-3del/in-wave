...
export default function Quote() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const horizontalLineWidth = horizontalLine ? (horizontalLine as HTMLElement).offsetWidth / 2 : 0;
      const coverTopWidth = window.innerWidth * 0.2;
      const coverTopLineMovementWidth = window.innerWidth * 0.08;

      if (!isDesk) {
        oddItems?.forEach((elem, i) => {
          gsap.set(elem, { x: (i + 1) * window.innerWidth });
        });
        ...
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            oddItems?.forEach((elem, i) => {
              gsap.to(elem, {
                x: (1 - progress) * (i + 1) * window.innerWidth,
                duration: 0.1
              });
            });
            ...
            if (vertical2) {
              gsap.to(vertical2, { y: -10 * rem * progress, duration: 0.1 });
            }
          }
        });
      } else {
        const item2Inner = quoteRef.current?.querySelector('.quote__item:nth-child(2) .quote__item__inner');
          ...
          onUpdate: (self) => {
            ...
            if (coverTopLine) {
              gsap.to(coverTopLine, {
                right: coverTopLineMovementWidth + coverTopLineMovementWidth * -progress,
                duration: 0.1
              });
            }
          }
    ...
    <section className="section quote-section">
      <div className="container">
        <div className="quote" ref={quoteRef}>
          ...
          <div className="quote__item">
            <span className="quote__item__inner">{t('line2')} <i>{t('line2Italic')}</i></span>
          </div>
          <div className="quote__item">
            <span className="quote__item__inner"><i>{t('line3Italic')}</i>{t('line3')}</span>
          </div>
          <div className="quote__item">
            <span className="quote__item__inner"><i>{t('line4Italic')}</i></span>
          </div>
